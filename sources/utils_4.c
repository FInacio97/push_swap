...
/*   Updated: 2023/11/04 13:01:48 by fda-estr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_shorts_index(t_data *data, int ind1, int ind2, int ind3)
{
	...
	while (current)
	{
		if (current->index == ind1 || current->index == ind2 || current->index == ind3)
			break ;
		data->top_head++;
...
void	phase_2_helper(t_data *data)
{
	if (data->s_a_head->index == data->ind_list->i - 2)
		move_ra(data);
	else if (data->s_a_tail->val != (data->ind_list->i - 2) && data->s_a_head->val > data->s_a_head->next->val)
		move_sa(data);
}
